...
1. **体验网页版**。对于*一键包用户*，直接运行`启动网页版.bat`或`启动网页版_持久化.bat`（推荐）即可启动网页版。启动脚本会在 PowerShell 窗口中显示启动进度，然后请手动打开浏览器访问 `http://127.0.0.1:8000`。**请根据提示配置API Key**。详细说明请参考 `USAGE_GUIDE.md`。
